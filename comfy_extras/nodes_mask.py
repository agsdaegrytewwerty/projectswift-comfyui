...
class GrowMask(IO.ComfyNode):
    ...
    def execute(cls, mask, expand, tapered_corners) -> IO.NodeOutput:
        try:
            ...
            raise RuntimeError("Mask morphology requires the optional scipy package") from error
        ...
        for m in mask:
            ...
            for _ in range(abs(expand)):
                if expand < 0:
                    output = ndimage.grey_erosion(output, footprint=kernel)
                else:
                    output = ndimage.grey_dilation(output, footprint=kernel)
