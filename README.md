# ProjectSwift Lean ComfyUI

This public fork is pinned to upstream ComfyUI commit
`78b43d25003ecf3c2b2509259d3a3cc72e941ec8` and intentionally supports only
ProjectSwift's SDXL image workflows (`standard`, `scratch`, `Matte`, and
`4to5`). Video, audio, 3D, partner/API nodes, browser UI packages, asset
indexing, and unrelated model families are omitted from release bundles.

Release `v1.0.0` contains a CPython 3.12 / CUDA 13.0 PyTorch 2.10 wheel built
only for NVIDIA compute capability 8.6 (`sm_86`, with no PTX fallback), plus a
single cold-start runtime bundle. It is CPU/import validated; a live RTX 3060
Ti GPU canary remains intentionally pending until ProjectSwift switches one
server to the separate lean startup path.

The fork remains licensed under upstream ComfyUI's GPL-3.0 license. See
[`UPSTREAM.md`](UPSTREAM.md) for provenance and [`LICENSE`](LICENSE) for terms.
