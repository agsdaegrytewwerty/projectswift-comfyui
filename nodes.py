...
async def init_builtin_extra_nodes():
    ...
    # Deliberately restricted to the nodes used by ProjectSwift's standard,
    # scratch, Matte and 4to5 SDXL image workflows.
    extras_files = [
...
async def init_extra_nodes(init_custom_nodes=True, init_api_nodes=True):
    ...
    for node_name in tuple(NODE_CLASS_MAPPINGS):
        if node_name not in allowed_nodes:
            NODE_CLASS_MAPPINGS.pop(node_name, None)
    for node_name in tuple(NODE_DISPLAY_NAME_MAPPINGS):
